      ...
      const createQueryBuilder = () => {
        return {
          ...
          limit: (_n: number) => {
            ...
            const result = Promise.resolve(getTableData(table));
            result.then = (onFulfilled: any, onRejected?: any) =>
              Promise.resolve(getTableData(table)).then(onFulfilled, onRejected);
            return result;
          },
      ...
      where: (_condition: any) => ({
        returning: () => {
          ...
          if (tableData.length === 0) {
            const inserted = { id: randomUUID(), ...values };
            tableData.push(inserted);
            return Promise.resolve([inserted]);
          }

          const updated = { ...tableData[0], ...values };
          tableData[0] = updated;
